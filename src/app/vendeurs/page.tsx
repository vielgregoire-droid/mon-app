...
export default function VendeursPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (country !== "ALL") {
      result = result.filter((s) => s.environment === country);
    }

    if (activeStatus) {
      result = result.filter((s) => s.pipeline_status === activeStatus);
    }
  ...
  // Sellers filtered by country only (for KPIs & pipeline, before status filter)
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 text-sm rounded-lg border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary w-52"
            />
          </div>
          ...
          <select
            ...
            className="px-4 py-2 text-sm rounded-lg border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
          >
            {COUNTRIES.map((c) => (
